...
unsigned int (__stdcall *_VirtualProtect)(void *lpAddress, unsigned int dwSize, unsigned int flNewProtect, unsigned int *lpflOldProtect);
unsigned long (__stdcall * _WriteMemory)(unsigned long offset, void *lpBuffer, unsigned long cb, unsigned long *lpcbBytesWritten);
void (__stdcall *_CopyMemory)(void *Destination, const void *Source, unsigned int Length);
...
unsigned int (__stdcall *_VirtualQuery)(void *lpAddress, PMEMORY_BASIC_INFORMATION lpBuffer, unsigned int dwLength);
unsigned int(__stdcall *_GetModuleBaseName)(void *hProcess, void *hModule, char *lpBaseName, unsigned int nSize);
