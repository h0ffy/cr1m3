...
/*	
	#pragma comment(linker, "/MERGE:.rdata=.data")  // merge bbs,rdata,data and text
	#pragma comment(linker, "/MERGE:.text=.data")	//		__JeNNYl4B__
	#pragma comment(linker, "/MERGE:.bbs=.data")	// reduce overhead
*/
...
int __cdecl main(int argc, char **argv) {
	...
	/*
	a = __findkern32();
	b = __find_function(a, 0xac4bf505);

	_printf("A: %#x\nB: %#x\n", a, b);
	_ExitProcess(0x00);
	*/

	if(SECURITY_STATUS==0x01) { // simple path point, if you like viruses learn to write a code
		__security();
	}
